<?php
/**
 * Monthly Financial Reports Model - V2
 ...
 */
public function create_monthly_report_v2($partner_id, $year, $month, $created_by, $description = '') {
    // Get partner name
    ...
    if($this->db->insert('monthly_financial_reports_v2', $data)) {
 ...
 */
public function get_monthly_report_v2($report_id) {
    // Get main report
    $report = $this->db->select('*')
        ->where('report_id', $report_id)
        ->get('monthly_financial_reports_v2')
        ->row_array();
 ...
 */
public function get_partner_monthly_reports_v2($partner_id, $status = null) {
    $this->db->select('r.*, p.name as partner_name, u.name as created_by_name, s.total_items')
        ->from('monthly_financial_reports_v2 r')
 ...
 * @return bool
 */
public function submit_monthly_report_v2($report_id, $submitted_by) {
    $data = array(
        'status' => 'submitted',
        'submitted_by' => $submitted_by,
        'submitted_at' => date('Y-m-d H:i:s')
    );
    return $this->db->update('monthly_financial_reports_v2', $data, array('report_id' => $report_id));
}
 ...
 * @return bool
 */
public function approve_monthly_report_v2($report_id, $approved_by, $notes = '') {
    $data = array(
        'status' => 'approved',
 ...
 * @return bool
 */
public function reject_monthly_report_v2($report_id, $comments) {
    $data = array(
        'status' => 'rejected',
...
public function generate_report_pdf($report_id) {
    $report = $this->get_monthly_report_v2($report_id);
    if(!$report) return FALSE;
