  ...
  (function () {
    const formValidationExamples = document.getElementById('formValidationExamples'),
      formValidationSelect2Ele = jQuery(formValidationExamples.querySelector('[name="formValidationSelect2"]')),
			formCurrency = jQuery(formValidationExamples.querySelector('[name="formCurrency"]')),
			formCategory = jQuery(formValidationExamples.querySelector('[name="formCategory"]')),
			formWorkPackage = jQuery(formValidationExamples.querySelector('[name="formWorkPackage"]')),
    ...
    const fv = FormValidation.formValidation(formValidationExamples, {
      fields: {
        formValidationName: {
          ...
          validators: {
            notEmpty: {
              message: 'Please enter Rates'
            },
            regexp: {
              regexp: /^[0-9]+$/,
              message: 'The Rates can only consist of numbers'
            }
          }
        },
				formCurrency: {
          ...
          validators: {
            notEmpty: {
              message: 'Please select the Work Package'
            }
          }
        },
          ...
          validators: {
            ...
            stringLength: {
              min: 50,
              max: 500,
              message: 'Description must be more than 50 and less than 500 characters long'
            }
          }
        },
        formValidationEmail: {
          ...
          rowSelector: function (field, ele) {
            ...
            switch (field) {
              case 'formValidationName':
							...
							case 'formAmount':
							case 'formRates':
							case 'formShortDescription':
              case 'formValidationEmail':
              case 'formValidationPass':
              case 'formValidationConfirmPass':
              case 'formValidationFile':
							case 'formValidationDate':
              case 'formValidationDob':
              case 'formValidationSelect2':
							case 'formCurrency':
							case 'formCategory':
							case 'formWorkPackage':
    ...
    // Flatpickr
    flatpickr(formValidationExamples.querySelector('[name="formValidationDate"]'), {
      enableTime: false,
      // See https://flatpickr.js.org/formatting/
      dateFormat: 'Y/m/d',
      // After selecting a date, we need to revalidate the field
      onChange: function () {
        fv.revalidateField('formValidationDate');
      }
    });
    ...
    if (formValidationSelect2Ele.length) {
      ...
      formCurrency
        ...
        .on('change.select2', function () {
          // Revalidate the color field when an option is chosen
          fv.revalidateField('formCurrency');
        });
    }

		if (formCategory.length) {
      formCategory.wrap('<div class="position-relative"></div>');
      formCategory
        .select2({
          placeholder: 'Select Category',
          dropdownParent: formCategory.parent()
        })
        .on('change.select2', function () {
          // Revalidate the color field when an option is chosen
          fv.revalidateField('formCategory');
        });
    }

		if (formWorkPackage.length) {
      formWorkPackage.wrap('<div class="position-relative"></div>');
      formWorkPackage
        .select2({
          placeholder: 'Select Work Package',
          dropdownParent: formWorkPackage.parent()
        })
        .on('change.select2', function () {
          // Revalidate the color field when an option is chosen
          fv.revalidateField('formCurrency');
